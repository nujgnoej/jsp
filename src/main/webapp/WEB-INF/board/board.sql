...
insert into board values (default,'관리맨','게시판 서비스를 시작합니다.','junmo8492@naver.com','http://blog.naver.com/junmo8492','이곳은 게시판입니다.',default,default,'192.168.50.112',default,'admin');

select * from board;

select * from board where idx = 5-1;	/* 현재글이 5번글인 경우 */
select * from board where idx = 1-1;	/* 현재글이 1번글인 경우 */
select * from board where idx = 21-1;
select * from board where idx = 21+1;
...
-- board테이블의 idx 37번글에 적혀있는 댓글(boardReply테이블)의 개수는?
select count(*) from boardReply where boardIdx = 30;
-- board테이블의 idx 37번글에 적혀있는 댓글(boardReply테이블)의 개수는?(단, 출력은 부모글의 고유번호와 댓글개수를 출력)
select boardIdx as 부모고유번호, count(*) from boardReply where boardIdx = 30;
-- board테이블의 고유번호가 30번 자료의 닉네임은?
select nickName from board where idx = 30;
-- board테이블 30번의 닉네임과 부모고유번호, board30번에 달려있는 댓글(boardReply)의 갯수를 출력?
select (select nickName from board where idx = 30) as 닉네임, boardIdx as 부모고유번호, count(*) from boardReply where boardIdx = 30;
-- board테이블의 30번 작성자의 아이디와, 현재글에 달려있는 댓글(boardReply) 사용자의 닉네임을 출력하시오?
select (select mid from board where idx = 30) as 작성자아이디, nickName from boardReply where boardIdx = 30;
select mid, (select nickName from boardReply where boardIdx = 30) as 작성자아이디 from board where idx = 30; /* 부모관점에서 보게되면 'where idx=30'의 결과는 1개만을 요구하기에 2개이상의 자료가 출력된다면 오류가 발생한다. */
-- board테이블의 30번 작성자의 아이디와, 현재글에 달려있는 댓글(boardReply)의 개수를 출력하시오?
select mid,(select count(*) from boardReply where boardIdx = 30) from board where idx = 30;

-- 부모테이블(board)에 있는 자료중 뒤에서 5개를 출력하되(아이디,닉네임), 부모테이블에 달려있는 댓글(boardReply테이블)의 수와 함께 출력하시오.

    ...
    <div>
    	<table class="table table-bordered" style="height:450px;">
    		...
    		<tr>
    			...
    			<!-- 시작일 이전을 공백을 이전달의 날짜로 채운다. -->
    			<c:forEach var="preDay" begin="${preLastDay - (startWeek - 2)}" end="${preLastDay}">
    				<td style="color:#aaa; font-size:0.6em;">${prevMonth+1}월${preDay}일</td>
    			...
    			<c:forEach begin="${nextStartWeek}" end="7" varStatus="nextDay">
    				<td style="color:#aaa; font-size:0.6em;">${nextMonth+1}월${nextDay.count}일</td>
    				<c:set var="cnt" value="${cnt + 1}"/>
    			</c:forEach>
    		</tr>
    	</table>

...
%>
	...
	<div class="text-center">
		<%if(pag != 1) { %>
			[<a href="adGuestList.ad?pag=1"><font color='black'>처음페이지</font></a>]
		<%} %>
		
		<%if(curBlock > 0) { %>
			[<a href="adGuestList.ad?pag=<%=(curBlock-1)*blockSize + 1%>"><font color='black'>이전블럭</font></a>]
		<%} %>
		<%
			for(int i=(curBlock*blockSize)+1; i<=(curBlock*blockSize)+blockSize; i++) {
				if(i > totPage) break;
				if(i == pag) out.println("[<a href='adGuestList.ad?pag="+i+"'><font color='black'><b>"+i+"</b></font></a>]");
				else out.println("[<a href='adGuestList.ad?pag="+i+"'><font color='black'>"+i+"</font></a>]");
			}
		%>
		<%if(curBlock < lastBlock) { %>
			[<a href="adGuestList.ad?pag=<%=(curBlock+1)*blockSize + 1%>"><font color='black'>다음블럭</font></a>]
		<%} %>
		
		<%if(pag != totPage) { %>
			[<a href="adGuestList.ad?pag=<%=totPage%>"><font color='black'>마지막페이지</font></a>]
		<%} %>
	</div>
	<!-- 블록 페이징 처리 끝 -->
</div>
